...
data "aws_ami" "ubuntu" {
  ...
  filter {
    name   = "name"
    values = ["ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-arm64-server-20231207"]
  }
  owners = ["099720109477"]
}
...
resource "aws_spot_instance_request" "default" {
  ...
  dynamic "ebs_block_device" {
    ...
    content {
      ...
      iops                  = lookup(ebs_block_device.value, "iops", null)
      kms_key_id            = var.kms_key_id == "" ? join("", aws_kms_key.default[*].arn) : lookup(aws_kms_key.default[0], "kms_key_id", null)
      snapshot_id           = lookup(ebs_block_device.value, "snapshot_id", null)
      volume_size           = lookup(ebs_block_device.value, "volume_size", null)
